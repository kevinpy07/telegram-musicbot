    ...
    if yt_cookies:
        try:
            ...
            lines = [
                ...
                "# Generated by Userbot on Startup",
                ""
            ]
            for part in yt_cookies.split(";"):
                part = part.strip()
                ...
                for domain in [".youtube.com", ".google.com"]:
                    lines.append(f"{domain}\tTRUE\t/\tTRUE\t{expiry}\t{key}\t{val}")
            with open(_0x_ck, "w", encoding="utf-8") as f:
                f.write("\n".join(lines) + "\n")
            _0x_log.info("Generated cookies.txt.")
        except Exception as e:
            _0x_log.error(f"Failed to generate cookies.txt: {e}")
...
async def _0x_search(query: str):
    def _extract():
        ...
        if os.path.exists(_0x_ck):
            ydl_opts['cookiefile'] = _0x_ck
        with YoutubeDL(ydl_opts) as ydl:
            try:
                info = ydl.extract_info(search_query, download=False)
                if not info:
                    if not is_link and search_query.startswith("ytsearch"):
                ...
                return {
                    ...
                    'url': result.get('webpage_url', query),
                    'stream_url': result.get('url'),
                    'uploader': result.get('uploader', 'Unknown Artist')
                }
            except Exception as e:
                _0x_log.error(f"Extraction error: {e}")
                if not is_link and search_query.startswith("ytsearch"):
                    try:
                        alt_query = f"ytmsearch1:{query}"
                        info = ydl.extract_info(alt_query, download=False)
